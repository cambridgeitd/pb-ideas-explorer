...
- **Full-text search** across ~9,000 idea titles, descriptions, and locations, with term highlighting
...
`scripts/build_data.py` transforms these into the compact JSON in `data/` that the site loads. It also:
...
- **Derives outcomes** — the `Idea Status` column is only populated for cycles 7 and 10, so a rules-based classifier also reads the free-text `Project Status` staff notes (2,300+ distinct strings) to assign each idea one of eight outcome categories (inspired a winning project, made the ballot, shortlisted, already underway, referred to City staff, not advanced, not eligible, no recorded outcome). ~84% of ideas get a meaningful category; the rest show "No recorded outcome."
...
1. **Idea status/outcome for all cycles** — `Idea Status` is only populated for PB7 and PB10. Backfilling the other cycles (even coarsely: won / ballot / shortlisted / not advanced / ineligible) would replace the fragile text-pattern classifier with authoritative data.
...
3. **PB11 (and future) ideas** — the ideas dataset currently ends at PB10, while the projects file already includes PB11. Ongoing exports for each new cycle would keep the site current.
4. **Winning project implementation status** — planned / in construction / completed (+ completion date, actual cost, photo). This would let residents see PB deliver, and would be great on the map.
5. **Per-cycle context** — PB budget amount, number of voters, and vote method per cycle, for a "PB by the numbers" trend panel.
6. **Consistent geocoding** — ~36% of ideas have no coordinates; many are city-wide, but a flag distinguishing "city-wide" from "not geocoded" (and geocoding where possible) would make the map more honest and complete.
7. **Category tags for ideas** — the committee assignment is one-dimensional; multi-tags (e.g., "trees", "bike safety", "accessibility") would sharpen search and trends.
8. **Ballot project descriptions ↔ ideas counts** — how many ideas fed each proposal, to highlight the most-requested concepts each cycle.

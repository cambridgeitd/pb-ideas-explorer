...
(async function () {
  ...
  function renderChart(rows) {
    ...
    CYCLES.forEach((c, ci) => {
      ...
      svg += `<g class="bar${selCls}" data-cycle="${c.num}">`;
      let y = H - padB;
      for (const k of keys) {
      ...
      if (totals[ci])
        svg += `<text class="total-label" x="${x + bw / 2}" y="${y - 5}">${totals[ci].toLocaleString()}</text>`;
    ...
    svgEl.addEventListener("mousemove", (e) => {
      ...
      const rowsHtml = [...g.querySelectorAll(".bar-seg")]
        ...
        .map(
          (r) =>
            `<div><span style="color:${r.getAttribute("fill")}">●</span> ${esc(groups[r.dataset.k].label)}: <b>${(+r.dataset.v).toLocaleString()}</b></div>`
        )
        .join("");
      tip.innerHTML = `<b>${c.label}</b> · ${esc(c.dates)}${rowsHtml ? "<hr style='border:0;border-top:1px solid #eee;margin:5px 0'>" + rowsHtml : "<div>No matching ideas</div>"}`;
